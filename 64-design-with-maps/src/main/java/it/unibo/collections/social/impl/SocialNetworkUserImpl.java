...
public final class SocialNetworkUserImpl<U extends User> extends UserImpl implements SocialNetworkUser<U> {
    ...
    public boolean addFollowedUser(final String circle, final U user) {
        Set<U> friendsCircle = this.friends.get(circle); 
        if(friendsCircle == null){
            friendsCircle = new HashSet<>();
            this.friends.put(circle, friendsCircle);
        }
    ...
    public List<U> getFollowedUsers() {
        final Set<U> followedFriends = new HashSet<>();
        for (final Set<U> group : friends.values()) {
            followedFriends.addAll(group);
        }
        return new ArrayList<>(followedFriends);
    }
}
